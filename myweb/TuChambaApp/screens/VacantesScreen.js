...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, ActivityIndicator, TextInput, RefreshControl, ScrollView } from 'react-native';
            ...
            <View style={styles.container}>
                ...
                <ScrollView contentContainerStyle={styles.detailContainer}>
                    ...
                    {(selected.requirements || []).length > 0 ? <>
                        ...
                        {selected.requirements.map((r, i) => <Text key={i} style={styles.listItem}>• {r}</Text>)}
                    </> : null}
                    {(selected.benefits || []).length > 0 ? <>
                        <Text style={styles.sectionTitle}>Ofrecemos</Text>
                        {selected.benefits.map((b, i) => <Text key={i} style={styles.listItem}>• {b}</Text>)}
                    </> : null}
                </ScrollView>
            </View>
        );
        ...
        <View style={styles.container}>
            ...
            {loading ? <ActivityIndicator size="large" color="#0a66c2" style={{ marginTop: 40 }} /> :
                <FlatList
                    ...
                    ListEmptyComponent={<View style={styles.empty}><Text style={styles.emptyText}>No hay vacantes</Text></View>}
                    renderItem={({ item }) => {
...
const styles = StyleSheet.create({
    ...
    header: { backgroundColor: '#0a66c2', padding: 16, paddingTop: 20 },
    ...
    card: { backgroundColor: 'white', margin: 10, marginTop: 0, borderRadius: 12, padding: 14, elevation: 2 },
